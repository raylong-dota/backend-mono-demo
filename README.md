...
# 2. 生成 gRPC / HTTP / Swagger 代码
cd app/order/service && make api
...
| `make lint` | 对全项目运行 golangci-lint（含自动修复，本地开发用） |
| `make lint-check` | 仅检查，不修复（CI / PR Gate 使用） |
| `make api` | 对所有服务生成 proto 代码 |
| `make wire` | 对所有服务生成 Wire 代码 |
| `make proto` | 对所有服务生成内部 proto struct |
...
| `protoc-gen-go` / `wire` / `golangci-lint` 等 | `@latest`（固化可在 `go.mod` 中锁定） |
