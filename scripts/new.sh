...
if [ -z "$SVCN" ]; then
    ...
    echo "Usage: make new svcn=<service-name>"
    echo "Example: make new svcn=order"
    exit 1
fi

if ! echo "$SVCN" | grep -qE '^[a-z][a-z0-9]*$'; then
...
SVCN_TITLE="$(echo "${SVCN:0:1}" | tr '[:lower:]' '[:upper:]')${SVCN:1}"
# order → ORDER  (enum values)
SVCN_UPPER="$(echo "$SVCN" | tr '[:lower:]' '[:upper:]')"

# ── Paths ───────────────────────────────────────────────────────────────────
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
TPL_APP="$ROOT_DIR/app/helloworld/service"
TPL_API="$ROOT_DIR/api/helloworld/service/v1"
DST_APP="$ROOT_DIR/app/$SVCN/service"
DST_API="$ROOT_DIR/api/$SVCN/service/v1"

if [ -d "$DST_APP" ]; then
    echo "Error: app/$SVCN/service already exists"
    exit 1
fi
...
cp "$TPL_API/greeter.proto"       "$DST_API/${SVCN}.proto"
...
cp "$TPL_APP/internal/biz/greeter.go"     "$DST_APP/internal/biz/${SVCN}.go"

# Service (transport handler) layer
cp "$TPL_APP/internal/service/service.go" "$DST_APP/internal/service/service.go"
cp "$TPL_APP/internal/service/greeter.go" "$DST_APP/internal/service/${SVCN}.go"

# Data access layer
cp "$TPL_APP/internal/data/data.go"       "$DST_APP/internal/data/data.go"
cp "$TPL_APP/internal/data/greeter.go"    "$DST_APP/internal/data/${SVCN}.go"
...
find "$DST_API" "$DST_APP" -type f | while read -r f; do
    ...
    sedi "s|app/helloworld/service|app/${SVCN}/service|g"       "$f"
    sedi "s|api/helloworld/service/v1|api/${SVCN}/service/v1|g" "$f"

    # Remaining helloworld occurrences (proto package declarations, go_package options)
    sedi "s/helloworld/${SVCN}/g" "$f"

    # Entity type names (Title case) — order matters: longer compound names first
    sedi "s/GreeterService/${SVCN_TITLE}Service/g"   "$f"
    sedi "s/GreeterUsecase/${SVCN_TITLE}Usecase/g"   "$f"
    sedi "s/GreeterRepo/${SVCN_TITLE}Repo/g"         "$f"
    sedi "s/Greeter/${SVCN_TITLE}/g"                 "$f"

    # Entity variable/function names (lowercase)
    sedi "s/greeterService/${SVCN}Service/g"         "$f"
    sedi "s/greeterUsecase/${SVCN}Usecase/g"         "$f"
    sedi "s/greeterRepo/${SVCN}Repo/g"               "$f"
    sedi "s/greeter/${SVCN}/g"                       "$f"

    # Enum values (UPPER case)
    sedi "s/GREETER/${SVCN_UPPER}/g"                 "$f"

    # Proto message names
    sedi "s/HelloRequest/${SVCN_TITLE}Request/g"     "$f"
    sedi "s/HelloReply/${SVCN_TITLE}Reply/g"         "$f"
done

echo ""
echo "✓ Service created:"
echo "  api/$SVCN/service/v1/"
echo "  app/$SVCN/service/"
echo ""
echo "Next steps:"
echo "  cd app/$SVCN/service"
echo "  make api     # generate protobuf (grpc + http + swagger + errors)"
echo "  make proto   # generate internal conf"
echo "  make wire    # generate dependency injection"
